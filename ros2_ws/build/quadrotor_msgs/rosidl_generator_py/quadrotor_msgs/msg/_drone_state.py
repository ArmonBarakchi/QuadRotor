...
class DroneState(metaclass=Metaclass_DroneState):
    ...
    def __init__(self, **kwargs):
        ...
        if 'omega' not in kwargs:
            self.omega = numpy.zeros(3, dtype=numpy.float64)
        else:
    ...
    def omega(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                assert value.dtype == numpy.float64, \
                    "The 'omega' numpy.ndarray() must have the dtype of 'numpy.float64'"
                assert value.size == 3, \
                    "The 'omega' numpy.ndarray() must have a size of 3"
                self._omega = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'omega' field must be a set or sequence with length 3 and each value of type 'float' and each double in [-.000000, .000000]"
        self._omega = numpy.array(value, dtype=numpy.float64)
